...
public class ClassConsequentFilter {
    ...
    private static Collection<ViolationInfo> getMethodViolations(ClassOrInterfaceDeclaration declaration, Condition<Method> methodCondition, StaticAnalysisRule rule) {
        ...
        if (methods.isEmpty()) {
            // element is missing
            return GeneralUtility.listOf(new ViolationInfo(declaration, collectionToString(GeneralUtility.listOf(methodCondition)), true));
        }
    ...
    private static Collection<ViolationInfo> getFieldViolations(ClassOrInterfaceDeclaration declaration, Condition<Field> fieldCondition, StaticAnalysisRule rule) {
        ...
        if (fields.isEmpty()) {
            // element is missing
            return GeneralUtility.listOf(new ViolationInfo(declaration, collectionToString(GeneralUtility.listOf(fieldCondition)), true));
        }
        if (rule != null) { // TODO: can it even be null?
